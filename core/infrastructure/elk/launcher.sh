...
if [[ ! -f /home/elastic/cert_bundle.zip ]]; then
  ...
  PWK=$(tr -dc A-Za-z0-9 </dev/urandom | fold -w 16 | head -n 1)
  KIBANA_PASSWORD="${KIBANA_PASSWORD:-$PWK}"
  echo "KIBANA_PASSWORD=$KIBANA_PASSWORD" >> /home/elastic/.env
  export KIBANA_PASSWORD

  # Generate new token
  KIBANA_SERVICE_TOKEN=$(elasticsearch-${EK_VERSION}/bin/elasticsearch-service-tokens create elastic/kibana default | cut -d '=' -f2)
